...
const DashboardEnhanced = ({ user, setAuth }) => {
    ...
    useEffect(() => {
        // Fetch regions
        axios.get('http://localhost:5000/drought/regions')
    ...
    const fetchAlerts = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/alerts/user/${user}`);
    ...
    const fetchUnreadCount = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/alerts/unread-count/${user}`);
    ...
    const handleExportCSV = () => {
        window.open(`http://localhost:5000/export/csv/${user}`, '_blank');
    };

    const handleExportJSON = () => {
        axios.get(`http://localhost:5000/export/json/${user}`)
            .then(res => {
                const blob = new Blob([JSON.stringify(res.data, null, 2)], { type: 'application/json' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `predictions_${user}_${new Date().toISOString().split('T')[0]}.json`;
                a.click();
    ...
    const markAlertRead = async (alertId) => {
        try {
            await axios.post(`http://localhost:5000/alerts/${alertId}/read`);
            fetchAlerts();
            fetchUnreadCount();
        } catch (err) {
            ...
            {showAlerts ? (
                ...
                <section>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h3 style={{ margin: 0, color: '#34495e' }}>📜 Prediction History</h3>
