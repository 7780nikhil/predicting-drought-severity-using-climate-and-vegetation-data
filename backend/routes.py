...
def predict():
    ...
    try:
        ...
        # Make prediction with error handling for scikit-learn compatibility
        try:
            prediction = model.predict(input_data)[0]
        except AttributeError as e:
            if 'monotonic_cst' in str(e):
                # Handle scikit-learn version compatibility issue
                # The model was trained with an older version, try alternative approach
                print(f"Warning: Model compatibility issue detected: {e}")
                # Try to make prediction anyway (the attribute might not be critical)
                prediction = model._predict(input_data)[0] if hasattr(model, '_predict') else model.predict(input_data)[0]
            else:
                raise e
    ...
    try:
        ...
        for pred in predictions:
            ...
            if 'created_at' in pred:
                pred['created_at'] = serialize_datetime(pred['created_at'])
...
def get_history():
    """
    Get prediction history using query parameters.
    """
    try:
    ...
    try:
        ...
        for alert in alerts:
            ...
            if 'created_at' in alert:
                alert['created_at'] = serialize_datetime(alert['created_at'])
